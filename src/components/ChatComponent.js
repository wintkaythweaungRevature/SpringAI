...
function ChatComponent() {
  ...
  const askAI = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "ai", text: "This feature is currently under maintenance. Please try again later.", isError: true },
      ]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="chat-wrapper">
      ...
      <div className="chat-messages">
        ...
        {messages.map((msg, i) => (
          <div key={i} className={`chat-message-row ${msg.role === "user" ? "row-user" : "row-ai"}`}>
            {msg.role === "ai" && (
            ...
            <div className={`chat-bubble ${msg.role === "user" ? "bubble-user" : msg.isError ? "bubble-error" : "bubble-ai"}`}>
