...
function MetaInsightCard({ title, value, deltaPct, sub, sparkValues, accent = '#1877F2' }) {
  return (
    <div style={{ display: 'flex', gap: 12, alignItems: 'stretch', padding: '16px 14px', background: '#fff', border: '1px solid #e8ecf1', borderRadius: 12, boxShadow: '0 1px 3px rgba(15, 23, 42, 0.06)', minHeight: 112 }}>
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
        ...
        {sub && <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 8, lineHeight: 1.45 }}>{sub}</div>}
      </div>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 12 }}>
        <MetaInsightCard title="Messages" value={fmtVal(messages)} deltaPct={dMsg} sub={pickMetric(m, ['fromFollowersPct']) != null ? `From followers ${pickMetric(m, ['fromFollowersPct'])}%` : null} sparkValues={sparkMsg} accent={fb} />
        <MetaInsightCard title="Interactions" value={fmtVal(interactions)} deltaPct={dInt} sub={pickMetric(m, ['interactionsFromFollowers']) != null ? `From followers ${fmt(pickMetric(m, ['interactionsFromFollowers']))}` : null} sparkValues={sparkInt} accent="#7c3aed" />
        <MetaInsightCard title="Page visits" value={fmtVal(visits)} deltaPct={dVis} sparkValues={sparkVis} accent="#0891b2" />
        <MetaInsightCard title="Follows" value={fmtVal(follows)} deltaPct={dFol} sub={pickMetric(data, ['unfollows']) != null ? `Unfollows ${fmt(pickMetric(data, ['unfollows']))}` : null} sparkValues={sparkFol} accent="#ea580c" />
      </div>
    </div>
  );
}
