...
const API = (process.env.NEXT_PUBLIC_API_BASE || '').replace(/\/$/, '') || 'https://api.wintaibot.com';
...
function mediaTypeMeta(raw) {
  const m = String(raw || 'post').toLowerCase();
  if (m.includes('video')) return { key: 'video', label: 'Video', short: 'V' };
  if (m.includes('image') || m.includes('photo') || m.includes('thumbnail')) return { key: 'image', label: 'Image', short: 'I' };
  if (m.includes('text') || m.includes('caption') || m.includes('tweet')) return { key: 'text', label: 'Text', short: 'T' };
  return { key: 'post', label: 'Post', short: 'P' };
}
                        ...
                        {posts.slice(0, 4).map((p, i) => {
                          ...
                          return (
                            <div
                              ...
                              title={`${mt.label} · ${(pcDot?.label || p.platform || 'platform')}`}
                            >
                              {mt.short}
                            </div>
                          );
                        ...
                        {sched.slice(0, 3).map((j, i) => {
                          ...
                          return (
                            <div
                              ...
                              title={`${mt.label} · ${(pcDot?.label || j.platform || 'platform')}`}
                            >
                              {mt.short}
                            </div>
                          );
