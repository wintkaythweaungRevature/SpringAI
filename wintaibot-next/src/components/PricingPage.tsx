...
export default function PricingPage({ onClose }: { onClose?: () => void }) {
  const { apiBase, token, user, refetchUser, isSubscribed } = useAuth();
  ...
  const rootStyle = onClose ? s.overlay : s.page;
  const panelStyle = onClose ? s.modal : { ...s.modal, maxWidth: 1100, maxHeight: 'none' };
        ...
        <div style={s.cardsRow}>
          {PLANS.map((plan) => {
            ...
            return (
              ...
              >
                {plan.badge && <div style={{ ...s.planBadge, background: plan.color }}>{plan.badge}</div>}
                ...
                {yearly ? (
                  <div style={s.yearlyNote}>
                    ${plan.yearlyTotal}/year total · saves ${(plan.monthlyPrice - plan.yearlyPrice) * 12}/yr vs
                    monthly
                  </div>
                ) : (
                ...
                <ul style={s.featureList}>
                  ...
                  {plan.missing.map((feature) => (
                    <li
                      key={feature}
                      style={{ ...s.featureItem, opacity: 0.35, textDecoration: 'line-through' }}
                    >
                ...
                {isCurrent ? (
                  <button
                    style={{ ...s.ctaBtn, background: '#e2e8f0', color: '#64748b', cursor: 'default' }}
                    disabled
                  >
                    Current Plan
                  </button>
